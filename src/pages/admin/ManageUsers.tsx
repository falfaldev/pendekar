  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-[2rem] overflow-hidden border border-slate-200 shadow-sm">
        ...
        {/* Mobile card list — tampil di bawah 1024px (semua HP & tablet kecil) */}
        <div className="block lg:hidden divide-y divide-slate-200">
          {users.map((u) => (
            <div key={u.id} className="px-4 py-4 flex items-center gap-3 min-w-0">
              ...
              <div className="flex-1 min-w-0 overflow-hidden">
                ...
                <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                  <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-full shrink-0 ${u.role === 'admin' ? 'bg-red-50 text-red-700' : 'bg-blue-50 text-blue-700'}`}>{u.role}</span>
                  <span className="text-[10px] text-slate-500">Lvl {u.level} · {u.points} Pts</span>
                </div>
              </div>
              <div className="flex gap-2 shrink-0">
                <button onClick={() => handleToggleRole(u.id, u.role)}
                  className="p-2.5 border border-slate-200 rounded-xl text-slate-600 bg-white min-w-[44px] min-h-[44px] flex items-center justify-center">
        ...
        {/* Desktop table — hanya di layar lebar (laptop/desktop) */}
