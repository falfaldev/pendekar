...
export default function Register({ onLoginSuccess }: RegisterProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const { needsConfirmation } = await api.register(name, email, password, age, gender);
      if (needsConfirmation) {
        setError('Akun berhasil dibuat. Silakan cek email untuk konfirmasi sebelum login.');
        navigate('/login');
      } else {
        onLoginSuccess();
        navigate('/dashboard');
      ...
      setError(err.message || 'Pendaftaran gagal. Coba hubungi Admin.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-2xl p-8 border border-white/20 relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-on-surface-variant text-xs mt-2 uppercase tracking-widest font-bold">
            Daftar Akun Baru Remaja Cerdas
          </p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="email"
              placeholder="Contoh: falah@email.com"
              value={email}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-primary text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-indigo-700 transition-colors shadow-lg shadow-primary/20 active:scale-[0.98] mt-4"
          >
            {isLoading ? (
